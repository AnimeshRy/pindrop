...
**Every new scanner needs a manifest entry.** `pindrop setup` installs from
`internal/toolinstall/manifest.json`, generated by the `sources` table in
`internal/toolinstall/genmanifest`. A scanner missing from it still works when the
user installs it themselves, but `pindrop setup` will not provide it and nobody
will notice until someone asks why. Add the entry, run `make manifest`, review the
digests. See [ADR 0010](../decisions/0010-managed-scanner-installation.md).
